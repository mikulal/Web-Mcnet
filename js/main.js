...
function initCanvas() {
  ...
  function mkNode() {
    return { x:Math.random()*W, y:Math.random()*H, vx:(Math.random()-.5)*.18, vy:(Math.random()-.5)*.18, r:Math.random()*1.2+.7, p:Math.random()*Math.PI*2, ps:.007+Math.random()*.007 };
  }
  function initNodes() { nodes=[]; for(var i=0;i<38;i++) nodes.push(mkNode()); }
  function draw() {
    ...
    for(var k=0;k<nodes.length;k++){
      var n=nodes[k]; n.p+=n.ps;
      var g=.5+.5*Math.sin(n.p), al=.09+g*.14;
      ctx.beginPath();ctx.arc(n.x,n.y,n.r*3,0,Math.PI*2);ctx.fillStyle='rgba(59,143,224,'+(al*.12)+')';ctx.fill();
...
function initFilter() {
  document.querySelectorAll('.filter-btn').forEach(function(btn){
    btn.addEventListener('click',function(){
      ...
      document.querySelectorAll('.proj-card').forEach(function(c){c.classList.toggle('hidden',s!=='vse'&&c.dataset.sector!==s);});
    });
  });
}
